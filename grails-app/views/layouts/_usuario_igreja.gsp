							...
							<g:each in="${sc.model.Igreja.list(sort:"cidade")}" var="igreja">
								<li class="icon icon-arrow-left">
									<g:if test="${session.user.temPermissaoPara(igreja)}">
										<a class="icon icon-display editable" href="#">${igreja.cidade}<span style=" float:right" class="glyphicon glyphicon-edit" hidden data-href="/sc/igreja/edit/${igreja.id}"></span></a>
										<div class="mp-level">
											<h2 class="icon icon-display creatable">Setores<span style="float:right;cursor:pointer" class="glyphicon glyphicon-plus" hidden data-href="/sc/setor/create?igreja=${igreja.id}"></span></h2>
